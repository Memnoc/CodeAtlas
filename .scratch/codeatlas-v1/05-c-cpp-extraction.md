...
- [x] C and C++ implement the parser interface with grammars compiled in
- [x] `#include "..."` directives resolve to edges between files inside the
      repo; system includes (`<...>`) are ignored or marked external, never
      dangling
- [x] A header and its implementation file are related by an edge (pairing by
      include and/or naming convention)
- [x] Function nodes with line ranges; call edges where the callee is
      resolvable within the repo
- [x] Fixture test covers a header/source pair, a repo-internal include
