...
- [x] Import statements resolve to `imports` edges between file nodes for
      TS/JS, including relative paths and index-file conventions
- [x] Exported symbols produce `exports` edges; function invocations produce
      `calls` edges where the callee is resolvable
- [x] Edge types carry the fixed weights defined by the contract structs
- [x] No edge in the emitted map references a missing node (referential
      integrity asserted in tests)
- [x] Fixture-repo test asserts specific expected edges, including one
