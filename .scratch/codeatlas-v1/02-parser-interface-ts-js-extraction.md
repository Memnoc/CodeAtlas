...
- [ ] A per-language parser interface exists; TS/JS is its first
      implementation, grammar compiled into the binary (no runtime downloads,
      ADR-0006)
- [ ] Function and class nodes carry typed IDs, line ranges, and `contains`
      edges from their file
- [ ] Every node gets a mechanical summary and `provenance: structural`
- [ ] A file with syntax errors or an unsupported extension still appears as
      a file node; the run completes
- [ ] Fixture-repo test asserts the expected function/class nodes and
