...
- [ ] Drilling into a region shows a back control that returns to the
      overview, and it is reachable by keyboard.
- [ ] The tour offers a previous step alongside the next one, disabled at the
      first step rather than absent — a control that appears and disappears
      is harder to aim at than one that greys out.
- [ ] Back from a file selection returns to the region view with the region
      still open, not to the overview: one step back, not all the way out.
- [ ] `Escape` does whatever the back control does, at every level.
- [ ] The control states where it goes ("Back to regions") rather than
      just "Back", since the same word at three depths means three things.
- [ ] Existing exits keep working — the breadcrumb and the click-the-canvas
